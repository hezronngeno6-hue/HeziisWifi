...
| Item | Why | Notes |
|---|---|---|
| **USB stick, 8 GB or bigger** | To boot the Ubuntu installer | **You do not have one plugged in yet.** This is the only thing missing. |
...
Total extra spend: about the price of a USB stick.
...
5. When it asks about the disk — **stop and decide**. Wiping the disk erases
   Windows. If you want to keep Windows, choose the manual partitioning option
   and install alongside it (`Install Ubuntu Server` → `Custom storage layout`).
